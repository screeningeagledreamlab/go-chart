...
func (bc BarChart) drawSeries(r Renderer, canvasBox Box, xrange, yrange Range, s Series, seriesIndex int) {
	s.Render(r, canvasBox, xrange, yrange, bc.styleDefaultsSeries(seriesIndex))
}

func (bc BarChart) drawMinMax(r Renderer, canvasBox Box, yr Range) {
	var barBox Box
	by := canvasBox.Bottom - yr.Translate(bc.Max)
	barBox = Box{
		Top:    by,
		Left:   canvasBox.Left,
...
func (bc BarChart) styleDefaultsSeries(seriesIndex int) Style {
	return Style{
		DotColor:    bc.GetColorPalette().GetSeriesColor(seriesIndex),
		StrokeColor: bc.GetColorPalette().GetSeriesColor(seriesIndex),
